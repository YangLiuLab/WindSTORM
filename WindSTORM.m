...
% title = 'Input';
% dims = [1 50];
% definput = {'.\data','EPFL_Microtubule_Alexa647.tif','10000','1.4','1.5','100'};
...
[imBS,imBG] = BgSub(imRAW);

%% save background-subtracted image
info = imfinfo([PathName,'\',Filename]);
imF = length(info);
imBS_tiff = uint16(imBS); 
for K=1:imF
   imwrite(imBS_tiff(:, :, K), [Filename(1:end-4),'_BS.tif'], 'WriteMode', 'append');
end
%%
